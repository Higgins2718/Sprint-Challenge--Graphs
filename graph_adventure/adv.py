...
def test():
    ...
    for move in universal_traversalPath:
        player.travel(move)
...
# we may need a travelpathfowards and travelpathbacktracking to eventually combine for travelpath
# Could optional backpath save this conflict inside recursive function?

# There's a weird error that can occur when backtracking many times
"""
File "adv.py", line 97, in dft
    return backtrack(current_room)
  File "adv.py", line 157, in backtrack
    return backtrack(chosen_room, backpath=traversalPath_backtracking)
  File "adv.py", line 157, in backtrack
    return backtrack(chosen_room, backpath=traversalPath_backtracking)
  File "adv.py", line 157, in backtrack
    return backtrack(chosen_room, backpath=traversalPath_backtracking)
  [Previous line repeated 1 more time]
  File "adv.py", line 141, in backtrack
    for exit in chosen_room.getExits():
AttributeError: 'NoneType' object has no attribute 'getExits'

KEEP BACKTRACKING
BACKPATH ['s', 's', 'e', 'e', 's', 'n']
TRAVPATH ['n', 'n', 'w', 'w', 'n', 's', 'e', 'e', 'e', 'n', 's', 'w', 'w', 'w']
YOU SHOULD TRAVEL  n

Should have gone west — correct move was at end of travpath, but not at backpath because I keep passing it in
...
#      010--011--006                     #
#                                        #
^ won't go down -15, -12 if you backtrack back to 000

Start depth first
When no unexplored exit rooms
Pop last off stack
BReadth frist
Until no unexplored exits
Pop last off 
Depth first
...
def backtrack(current_room):
    ...
    '''
   # print("NO UNVISITED EXITS IN ROOM", str(current_room))
    #print("JUNCTIONPATH ", junction_path)
    #junction_path_copy = copy.deepcopy(junction_path)
    new_room = current_room
    while len(junction_path) > 0:
        next = junction_path.pop(-1)
        if new_room.getRoomInDirection(next) is not None:
            new_room = new_room.getRoomInDirection(next)
            traversalPath.append(next)
    ...
    print("NEW LOCATION ", str(new_room))
    print("JUNCTION PATH", junction_path)
    ...
    for direction in traversalPath:
        traversalPath_backtracking.append(find_reverse(direction))

    traversalPath.pop(-1)
    directions = ['n', 's', 'e', 'w']
    print("CURRENTLY IN ", str(current_room))
    print("JUNCTION ", junction_path)
    ...
    while len(junction_path) > 0:
        next_move = junction_path.pop(-1)
        print("NEXT ", next_move)
        if current_room is not None:
            pass
        else:
            print("CURRENT ROOM IS NONE")
        chosen_room = current_room.getRoomInDirection(next_move)
        if chosen_room is not None:
            print("YOU MOVED ", next_move, " INTO ", str(chosen_room))
        else:
            print("CHOSEN ROOM IS NONE")
            print("ROOM NUMBER, ", str(current_room), "HAS NO EXIT IN DIRECTION ", next_move)
            #print("AND CURRENT ROOM ", str(current_room))

        unvisited_rooms = []
        for exit in chosen_room.getExits():
            a_room = chosen_room.getRoomInDirection(exit)
            if a_room not in visited_rooms:
                unvisited_rooms.append(exit)

        if len(unvisited_rooms) > 0:
            print("GO BACK TO DFT")
            traversalPath.append(unvisited_rooms[0])
            s.push(chosen_room.getRoomInDirection(unvisited_rooms[0]))
            dft(chosen_room.getRoomInDirection(unvisited_rooms[0]))
        else:
            print("KEEP BACKTRACKING")
            print("REMAINING PATH ", junction_path)
    '''

    '''
    #while my_coords != dest_coords:
    if my_coords[0] < dest_coords[0]:
        #while my_coords[0] < dest_coords[0]:
            #pass
        # Try to go east:
        if current_room.getRoomInDirection('e') is not None:
            test_room = current_room.getRoomInDirection('e')
            if test_room == last_viable:
                print("FOUND IT")
                s.push(test_room)
                dft(test_room)
        ...
        # Try to go west:
        if current_room.getRoomInDirection('w') is not None:
            test_room = current_room.getRoomInDirection('w')
            if test_room == last_viable:
                print("FOUND IT")
                s.push(test_room)
                dft(test_room)
    if my_coords[1] < dest_coords[1]:
        #while my_coords[1] < dest_coords[1]:
            #pass
        # Try to go north:
        if current_room.getRoomInDirection('n') is not None:
            test_room = current_room.getRoomInDirection('n')
            if test_room == last_viable:
                print("FOUND IT")
                s.push(test_room)
                dft(test_room)
    else:
        print(my_coords[1], " go to ", dest_coords[1])
        #print("HER IT IS")

        # Try to go south:
        if current_room.getRoomInDirection('s') is not None:
            test_room = current_room.getRoomInDirection('s')
            if test_room == last_viable:
                print("FOUND IT")
                s.push(test_room)
                dft(test_room)
    ...
    for exit in next_room.getExits():
        ...
        a_room = next_room.getRoomInDirection(exit)
        if a_room not in visited_rooms:
            unvisited_rooms.append(exit)
    
    if len(unvisited_rooms) > 0:
        print("YOU CAN GO HERE ", unvisited_rooms)
        traversalPath.clear()
        # Breadth first — get last?
        next_room = next_room.getRoomInDirection(unvisited_rooms[-1])
        s.push(next_room)
        print("EXITING TO ", str(next_room))
        dft(next_room)
    else:
        print("NO UNVISITED ROOMS IN", next_room.getExits())
        print("MOST RECENT IN ", exit, " direction")

        for exit in next_room.getExits():
            if next_room.getRoomInDirection(exit) == recent_rooms[-1]:
                print("You just visited the room in ", exit, " direction")
            else:
                print(exit, " is a viable option")
                new_room = next_room.getRoomInDirection(exit)
                s.push(new_room)
                dft(new_room)
        return backtrack(next_room, backpath=traversalPath_backtracking, q=q, recent_rooms=recent_rooms, last_viable=last_viable)
    '''
...
'''
# IF CAN MOVE
for exit in v.getExits()[0]:
    print("EXIT", exit)
    new_room = player.currentRoom.getRoomInDirection('n')
    visited_rooms.add(new_room)
    player.travel('n')
    traversalPath.append('n')
# IF CAN MOVE
for exit in player.currentRoom.getExits()[0]:
    print(exit)
    new_room1 = player.currentRoom.getRoomInDirection('n')
    visited_rooms.add(new_room1)
    player.travel('n')
    traversalPath.append('n')


# HIT DEAD-END
# GO IN REVERSE DIRECTION OF LAST ACTION (Store last action?); implement get inverse of last func
for exit in player.currentRoom.getExits():
    print("DEAD END", exit)
    print("VISITED", visited_rooms)
    if player.currentRoom.getRoomInDirection(exit) in visited_rooms:
        print("YOU HAVE VISITED", exit)
        traversalPath_copy = traversalPath
        most_recent = traversalPath_copy.pop(-1)
        best_move = find_reverse(most_recent)
        print("YOU SHOULD TRAVEL ", best_move)
        #player.travel('s')
        #traversalPath.append('n')
    else:
        print("YOU HAVE NOT VISITED", exit)
# LAST ONE WAS MOST USEFUL MICRO EXAMPLE
# BACK AT STARTING ROOM
for exit in player.currentRoom.getExits():
...
'''
"""
def backtrack(current_room, backpath=None):
    traversalPath_backtracking = []
    # traversalPath_copy = traversalPath_backtracking.copy()
    # most_recent = traversalPath_copy.pop(-1)
    # maybe try while loop here
    # while room exists are visited?
    # This copy might be overwritten wheen backtrack is called 2nd time
    s_copy = copy.deepcopy(s)
    # most_recent = traversalPath_backtracking.pop(-1)

    if backpath is not None:
        traversalPath_backtracking = backpath
    else:
        traversalPath_copy = copy.deepcopy(traversalPath)
        # Might have to reverse resulting list?

        for item in traversalPath_copy:
            traversalPath_backtracking.append(find_reverse(item))
    print("BACKPATH", traversalPath_backtracking)
    print("TRAVPATH", traversalPath)

    best_move = traversalPath_backtracking.pop(-1)
    print("YOU SHOULD TRAVEL ", best_move)
    if current_room is not None:
        print("CURRENT ROOM IS ", str(current_room))
    else:
        print("CURRENT ROOM IS NONE")
    chosen_room = current_room.getRoomInDirection(best_move)
    s_copy.push(chosen_room)
    if chosen_room is not None:
        print("CHOSEN ROOM IS ", str(current_room))
    else:
        print("CHOSEN ROOM IS NONE")
        print("TRIED TO MOVE ", best_move)
        print("AND CURRENT ROOM ", str(current_room))
    ...
    if len(unvisited_rooms) > 0:
        print("GO BACK TO DFT")
        # TESTING; this may cause backtracking problems with infinite recursive loops
        traversalPath.append(unvisited_rooms[0])
        s.push(chosen_room.getRoomInDirection(unvisited_rooms[0]))
        dft(chosen_room.getRoomInDirection(unvisited_rooms[0]))
    else:
        print("KEEP BACKTRACKING")
        # TESTING; this may cause backtracking problems with infinite recursive loops
        traversalPath.append(best_move)
        return backtrack(chosen_room, backpath=traversalPath_backtracking)
"""